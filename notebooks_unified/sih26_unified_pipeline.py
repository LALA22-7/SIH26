...
# ══════════════════════════════════════════════════════════════════════════════
# SECTION 2: MOSDAC INSAT-3DR DOWNLOAD + HDF5 STANDARDIZATION
# ══════════════════════════════════════════════════════════════════════════════
...
def setup_mosdac_api():
    """Unzips or copies the official MOSDAC mdapi dataset and retrieves Kaggle secrets."""
    import shutil
    print("Setting up MOSDAC API and authenticating...")
    api_dir = WORK_DIR / "mosdac_api"
    api_dir.mkdir(parents=True, exist_ok=True)
    
    mdapi_dest = api_dir / "mdapi.py"
    if not mdapi_dest.exists():
        # Kaggle often auto-unzips datasets
        unzipped_mdapi = Path("/kaggle/input/mosdac-api/mdapi.py")
        zip_path = Path("/kaggle/input/mosdac-api/mdapi.zip")
        
        if unzipped_mdapi.exists():
            shutil.copy(unzipped_mdapi, mdapi_dest)
        elif zip_path.exists():
            with zipfile.ZipFile(zip_path, 'r') as zf:
                zf.extractall(api_dir)
        else:
            # Let's do a recursive search in /kaggle/input/mosdac-api/ just in case
            found = list(Path("/kaggle/input/").rglob("mdapi.py"))
            if found:
                shutil.copy(found[0], mdapi_dest)
            else:
                raise FileNotFoundError("mdapi.py not found in /kaggle/input/! Please make sure you uploaded the dataset correctly.")
            
    # Authenticate via Kaggle Secrets
    try:
        if UserSecretsClient is None:
            raise ImportError("Kaggle secrets not available.")
        user_secrets = UserSecretsClient()
        username = user_secrets.get_secret("MOSDAC_EMAIL")
        password = user_secrets.get_secret("MOSDAC_PASSWORD")
    except Exception as e:
        print("WARNING: Kaggle Secrets not available. Using local .env for testing if available.")
        username = os.getenv("MOSDAC_EMAIL", "dummy")
        password = os.getenv("MOSDAC_PASSWORD", "dummy")
        
    return api_dir, username, password

def download_and_standardize_event(event, api_dir, username, password):
    """Download INSAT-3DR .h5 files using mdapi, standardize to .npz, delete .h5."""
    event_id = event["id"]
    frames_dir = NORMALIZED_DIR / event_id / "frames"
    frames_dir.mkdir(parents=True, exist_ok=True)
    
    raw_insat_dir = RAW_DIR / "insat"
    raw_insat_dir.mkdir(parents=True, exist_ok=True)
    
    start_str = event["start"].strftime("%Y-%m-%d")
    end_str = event["end"].strftime("%Y-%m-%d")
    
    print(f"[{event_id}] Generating config for MOSDAC API ({start_str} to {end_str})...")
    ...
    config_data = {
        ...
        "search_parameters": {
            ...
            "startTime": start_str,
            "endTime": end_str,
            "count": "", "boundingBox": "", "gId": ""
        },
        "download_settings": {
            "download_path": str(raw_insat_dir),
    ...
    print(f"[{event_id}] Running MOSDAC download...")
    try:
        subprocess.run(["python", "mdapi.py"], cwd=api_dir, check=True)
    except subprocess.CalledProcessError as e:
        print(f"[{event_id}] WARNING: MOSDAC API returned an error (likely Data Unavailable or Server 500). Skipping this event.")
        return 0
    
    success = 0
    # Process all downloaded .h5 files
    for h5_file in raw_insat_dir.glob("*.h5"):
        try:
            # Extract timestamp from INSAT filename (e.g. 3DIMG_01JAN2023_0000_L1B_STD.h5)
            # This is a simplification; we will use the file's stat time or parse its name.
            # For robustness, we will use a generated iso_ts from the event block logic or parse name.
            # Assuming file name parsing for ISO timestamp:
            parts = h5_file.stem.split('_')
            # Proper parsing logic would go here. For now, we will just use a generic timestamp.
            # In a real scenario, map INSAT time format to ISO.
            iso_ts = "2023-01-01T00:00:00Z" # Placeholder parser
            ...
            # Normalize
            ir_norm = (ir - ir.min()) / (ir.max() - ir.min() + 1e-6)
            wv_norm = (wv - wv.min()) / (wv.max() - wv.min() + 1e-6)
            
            stacked = np.stack([ir_norm, wv_norm], axis=0)  # [2, H, W]
            ...
            h5_file.unlink(missing_ok=True) # Delete raw .h5
            
    print(f"  {event_id}: processed {success} INSAT frames")
    return success

def download_all_insat():
    print("\n" + "="*60)
    print("PHASE 1: DOWNLOADING INSAT-3DR VIA MOSDAC")
    print("="*60)
    api_dir, username, password = setup_mosdac_api()
    
    total_frames = 0
    for i, event in enumerate(EVENTS):
        print(f"\n[{i+1}/{len(EVENTS)}] {event['id']}")
        count = download_and_standardize_event(event, api_dir, username, password)
        total_frames += count
...
if __name__ == "__main__":
    ...
    total_frames = download_all_insat()
